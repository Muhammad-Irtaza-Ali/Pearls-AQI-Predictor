...
class BaseClient(ABC):
    ...
    async def fetch(self, http_client: httpx.AsyncClient, city: City) -> FetchResult:
        async with self._semaphore:
            logger.info("Starting %s for %s", self.source_name, city.name)
            started_at = time.perf_counter()
            collected_payloads: dict[str, Any] = {}
            errors: list[str] = []

            for label, url, params in self.request_specs(city):
            ...
            try:
                record = self.normalize(city, collected_payloads, response_time_seconds)
